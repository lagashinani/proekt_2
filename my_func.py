...
def get_token_link(app_id=7738389, scope=4):
    """Возвращает URL: строку для получения токена VK и пытается открыть браузер с этой строкой"""
...
def auth_handler():
    ...
    """
    save_device = input('Сохранить это устройство в доверенных (да/нет)?:\n')
    key = input('Введите одноразовый код:\n')
    remember_device = True if save_device.lower().strip() == 'да' else False
...
def get_user_info(api):
    """Возвращает словарь, c информацией о пользователе в формате:
    {'age': user_age, 'sex': user_sex, 'city': user_city}"""
...
def get_comments_count(user_id, api):
    ...
    if comments['count'] > 100:  # Если комментариев больше 100, получаем все вплоть до 10000 (макс. в ВК)
        ...
        for i in range(100, max_comment, 100):
            comments = api.photos.getAllComments(owner_id=user_id, count=100, offset=i)
...
def sql_connection(db_name):
    """Соединение с БД SQLlite"""
    try:
        con = sqlite3.connect(db_name)
        return con
    except sqlite3.Error:
        print('Ошибка базы данных: ', sqlite3.Error)


def write_db(data, db_name='db.sqllite'):
    """Выполняет добавление результатов поиска в БД"""
    con = sql_connection(db_name)
    cursor = con.cursor()
    for key in data:
        for photo in data[key]:
            values = (key, photo)
            cursor.execute('''INSERT INTO vk_users(user_url, photo_url) VALUES(?, ?)''', values)
    con.commit()


def read_users(db_name='db.sqllite'):
    """Считывает из БД и возвращает список из url страниц пользователей"""
    con = sql_connection(db_name)
    cursor = con.cursor()
    cursor.execute('SELECT DISTINCT user_url FROM vk_users;')
